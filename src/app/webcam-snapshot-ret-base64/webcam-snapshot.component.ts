...
export class WebcamSnapshotComponent implements AfterViewInit {
  ...
  renderFrame() {
    ...
    if (video_element.readyState === video_element.HAVE_ENOUGH_DATA) {
      ...
      this.dataService.testPostRequestImgRes(base64Image).subscribe((data: any[])=>{
        ...
        this.image.src = "data:image/png;base64," + data['face_img_base_64']

        // this.drawCanvasImage(this.image, this.canvas.nativeElement.width,
  ...
  drawCanvasImageWithFaceRects(image: any, width: number, height: number, face_boxes: any[]) {
    ...
    if(face_boxes != null)
    {
      ...
      for (let i = 0; i < face_boxes.length; i++) {
        ...
        var xmax = current_face_box[2] / 2.5;
        var ymax = current_face_box[3] / 1.7;
        //1.7

        this.context.beginPath();
        this.context.rect(xmin, ymin, xmax, ymax);
